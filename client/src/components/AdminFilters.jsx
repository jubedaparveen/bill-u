...
const AdminFilters = ({
  ...
  <div style={{ boxShadow: "0px 2px 10px 0px rgba(0,0,0,0.20)" }}>
    {/* Desktop/Large layout - unchanged */}
    <div className="hidden lg:flex flex-row justify-between gap-4 py-5 px-10 rounded-lg items-center">
    ...
    <div className="lg:hidden flex flex-col gap-3 py-5 px-4 sm:px-6 rounded-xl">
      <div className="relative w-full">
        ...
        <input
          ...
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-bill-yellow focus:border-transparent"
        />
      </div>
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        ...
        <div className="relative" ref={statusDropdownRef}>
          <button
            onClick={() => setStatusFilter((prev) => (prev === "open" ? "closed" : "open"))}
            className="w-full flex items-center justify-between px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-all duration-200"
          >
            <span>{statusFilter}</span>
            <svg className={`w-4 h-4 ml-2 transition-transform duration-200 ${statusFilter === "open" ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        ...
        <button
          onClick={handleResetFilters}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border text-sm border-gray-200 hover:bg-gray-200 text-gray-600 rounded-xl transition-colors duration-200"
        >
          <RotateCcw className="w-4 h-4" />
          Reset Filters
        </button>
